...
function main() {
  ...
  [[ x$SKIP_OR_INSTALL == "x1" ]] && {
    ...
    set_permissions
    sudo mkdir -p /data/var/lib/openreplay
    sudo cp -f openreplay-cli /bin/openreplay
    install_openreplay
    # If you install multiple times using init.sh, Only keep the latest installation
    if [[ -d /data/var/lib/openreplay/openreplay ]]; then
      cd /data/var/lib/openreplay/openreplay
      date +%m-%d-%Y-%H%M%S | sudo tee -a /data/var/lib/openreplay/or_versions.txt
      sudo git log -1 2>&1 | sudo tee -a /data/var/lib/openreplay/or_versions.txt
      sudo rm -rf /data/var/lib/openreplay/openreplay
      cd -
    fi
    sudo cp -rf $(cd ../.. && pwd) /data/var/lib/openreplay/openreplay
    sudo cp -rf ./vars.yaml /data/var/lib/openreplay/
  }
}

main

info "Configuration file is saved in /data/var/lib/openreplay/vars.yaml"
info "You can delete the directory $(echo $(cd ../.. && pwd)). Backup stored in /data/var/lib/openreplay"
